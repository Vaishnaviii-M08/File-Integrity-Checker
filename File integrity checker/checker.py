...
class FileIntegrityChecker:
    ...
    def monitor_directory(self, interval=10):
        """Monitor the directory for changes and check integrity periodically."""
        while True:
...
if __name__ == "__main__":
    # Get directory to monitor from the user
    directory_to_monitor = input("Enter the directory to monitor: ").strip()
    
    # Ask for the file name to store hashes
    hash_file = input("Enter the hash file name (default 'file_hashes.json'): ").strip() or "file_hashes.json"
    
    # Create an instance of FileIntegrityChecker and monitor the directory
    checker = FileIntegrityChecker(directory_to_monitor, hash_file)
    checker.monitor_directory(interval=10)  # Monitor every 10 seconds
